...
struct TestCardView: View {
    ...
    var body: some View {
        ...
        return TempCardView()
                ...
                // 크기 조정
                .scaleEffect((viewState.height + dragState.translation.height < 0 && !dragState.isDragging) ? 2.5 : 1)
                .animation(.easeInOut)
                // 위치 설정
                .offset(
                    // 가로는 고정
                    x: viewState.width,
                    // 세로는 드래그를 멈추는 위치에 따라
                    y: !dragState.isDragging ? (isInCardZone() ? -largeCardHeight/2 + cardZoneHeightOverMiddle : initialCardLocation) : viewState.height + dragState.translation.height
                )
                .shadow(radius: dragState.isActive ? 8 : 0)
                .animation(Animation.linear(duration: minimumLongPressDuration), value: viewState)
